# Task Breakdown - [Date]

## Instructions
**What**: Detailed action steps
**Include**:
- Break each plan item into specific steps
- Add links and resources
- Clear instructions for execution

---

## Task 1: [Task Name from plans.md] 
More than 20 leads
### Steps:
1. [Use Re extensions]
2. Speed is needed to accomplish this]
3. [Accuracy is also impoertant]

### Resources and Links:
- [Resource name](URL)
- [Tool or Document](URL)

### Instructions:

Detailed instructions for execution...
Pay cose attention and cpeed up work to accomplish task.

---

## Task 2: [Task Name from plans.md]
Send messages
### Steps:
1. [Check for connetced as i go by my work and send 1st message to them. ]
2. [Make sure the correct name of the lead is attached]
3. [Specific step 3]

### Resources and Links:
- [Resource name](URL)
- [Tool or Document](URL)

### Instructions:

Detailed instructions for execution...


---

## Task 3: [Task Name from plans.md]

### Steps:
1. [Specific step 1]
2. [Specific step 2]
3. [Specific step 3]

### Resources and Links:
- [Resource name](URL)
- [Tool or Document](URL)

### Instructions:

Detailed instructions for execution...


---

## Reminder
- Break down each plan into steps
- Add all necessary links and resources
- Write clear execution instructions
